...
namespace ProjetoBiblioteca.Controllers
{
    public class EmprestimoController : Controller
    {
        ...
        public IActionResult Vitrine(string? q)
        {
            ...
            using (var cmd = new MySqlCommand("sp_vitrine_buscar", conn)
            ...
            // 2) títulos para o datalist
            using (var cmdAll = new MySqlCommand("sp_vitrine_buscar", conn)
            {
                CommandType = CommandType.StoredProcedure
            })
            {
        ...
        // =================== Helpers simples do carrinho (string CSV ↔ List<int>) ===================
        ...
        public IActionResult Carrinho()
        {
            ...
            if (ids.Count > 0)
            {
                ...
                using var conn = db.GetConnection();
                using (var cmd = new MySqlCommand("sp_livro_listar_por_ids", conn)
                { CommandType = CommandType.StoredProcedure })
                {
                ...
                {
                    while (rd2.Read())
                        leitores.Add((rd2.GetInt32("id"), rd2.GetString("nome")));
                }
        ...
        public IActionResult Finalizar(int idLeitor, DateTime dataPrevista)
        {
            ...
            try
            {
                ...
                int idEmp;
                using (var cmd = new MySqlCommand("sp_emprestimo_criar", conn, tx)
                { CommandType = CommandType.StoredProcedure })
                {
                ...
                {
                    using var cmdI = new MySqlCommand("sp_emprestimo_adicionar_item", conn, tx)
                    { CommandType = CommandType.StoredProcedure };
